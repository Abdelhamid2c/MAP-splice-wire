...
# def inserer_code_barre_128(donnees, ws, cell_pos="B2", width=None, height=None):
#     """
#     Génère un code-barres Code128 à partir de 'donnees' et l'insère dans la cellule Excel spécifiée.

#     :param donnees: Texte à encoder (ex : 'ABC123456')
#     :param ws: Feuille de calcul Excel où insérer le code-barres
#     :param cell_pos: Position de la cellule où insérer l'image (ex: "B2")
#     :param width: Largeur souhaitée pour l'image (None = taille originale)
#     :param height: Hauteur souhaitée pour l'image (None = taille originale)
#     """
#     # 1. Créer un fichier temporaire avec un nom valide
#     with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp:
#         temp_filename = tmp.name
    
#     # 2. Générer le code-barres en image
    
#     writer = ImageWriter()
#     writer.text = False
#     code128 = Code128(donnees, writer=writer)

#     fichier_image = code128.save(temp_filename.replace('.png', ''))
...
#         # Si une seule dimension est spécifiée, calculer l'autre pour conserver les proportions
#         if width is None:
...
#         resized_img.save(fichier_image)
        
#         print(f"Image redimensionnée: {width}x{height} pixels")
    
#     # 4. Insérer l'image dans Excel
#     cell_excel = ws.range(cell_pos)
#     picture = ws.pictures.add(
#         fichier_image, 
#         top=cell_excel.top, 
#         left=cell_excel.left,
#         width=width,
#         height=height
#     )
    
#     # 5. Ajouter le texte sous le code-barres (optionnel)
#     # Hauteur approximative de l'image pour placer le texte en dessous
#     # Si la hauteur est définie, utiliser cette valeur pour calculer le décalage
#     row_offset = 2
#     if height is not None:
#         # Ajuster le décalage en fonction de la hauteur (approximation)
#         row_offset = max(2, int(height / 20))
    
#     text_cell = cell_excel.offset(row_offset=row_offset)
...
#     # Insertion du code-barres avec redimensionnement
#     inserer_code_barre_128("abdelhamid_chebel", ws, "B2", width=150, height=50)
...
barcode = Code128BarcodeGenerator("abdelhamid, Chebel!")
barcode.generate()
...
cell_excel = ws.range('V2')
left = cell_excel.left
top = cell_excel.top
cell_width = cell_excel.width
cell_height = cell_excel.height


img = Image.open(os.path.abspath(output))
resized_img = img.resize((200, 60), Image.LANCZOS)
resized_img.save(os.path.abspath(output))
pic = ws.pictures.add(
        os.path.abspath(output),
        left=left,
        top=top)

pic.left = left + (cell_width - pic.width) / 2
pic.top = top + (cell_height - pic.height) / 2

# Optional: Make cell taller if needed to fit the image
if pic.height > cell_height:
    ws.range("V2").row_height = pic.height + 5
